...
WEEKEND_WORKING_HOURS_START_HOUR=11
WEEKEND_WORKING_HOURS_END_HOUR=21
SEND_INTERVAL_MINUTES=3
```

Weekend window: Saturday and Sunday use `WEEKEND_WORKING_HOURS_START_HOUR`-`WEEKEND_WORKING_HOURS_END_HOUR`, default `11:00-21:00`.
...
Скрипт создаст две колонки статусов и будет отправлять сообщения только с 08:00 до 18:00 по болгарскому времени. Если лид появился в нерабочее время, статусы останутся пустыми, а time-based trigger отправит сообщения утром после 08:00.

Если образовалась очередь, скрипт отправляет только одно сообщение за запуск и выдерживает паузу `SEND_INTERVAL_MINUTES` между отправками. Для повторной отправки очистите статус нужного канала.
